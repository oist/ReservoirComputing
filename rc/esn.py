...
class ESNConfig:
    ...
    weights_generation_strategy: (
        Literal["Gaussian", "Uniform", "Bernoulli", "Small-World", "Scale-Free"]
        | Callable
    ) = "Gaussian"
    bias_generation_strategy: Literal["Gaussian", "Uniform", "Bernoulli"] | Callable = (
        "Uniform"
    )
    input_generation_strategy: (
        Literal["Gaussian", "Uniform", "Bernoulli"] | Callable
    ) = "Uniform"
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.input_dim, (int, np.integer)) or self.input_dim <= 0:
            raise ValueError(
                f"input_dim must be a positive integer, got {self.input_dim}"
            )

        # validate spectral_radius
        if (
            not isinstance(self.spectral_radius, (int, float))
            or self.spectral_radius <= 0
        ):
            raise ValueError(
                f"spectral_radius must be positive, got {self.spectral_radius}"
            )
        ...
        if not isinstance(self.input_scaling, (int, float)) or self.input_scaling < 0:
            raise ValueError(
                f"input_scaling must be non-negative, got {self.input_scaling}"
            )
        if not isinstance(self.bias_scaling, (int, float)) or self.bias_scaling < 0:
            raise ValueError(
                f"bias_scaling must be non-negative, got {self.bias_scaling}"
            )

        # validate seed
        if self.seed is not None and (
            not isinstance(self.seed, (int, np.integer)) or self.seed < 0
        ):
            raise ValueError(
        ...
        if not isinstance(self.leaky_rate, (int, float)) or not (
            0 < self.leaky_rate <= 1
        ):
...
class ESN:
    ...
    def __init__(
        ...
        dynamics: ReservoirDynamics | None = None,
        *,
        N: int | None = None,
        ...
        **kwargs,
        ...
        if config is not None:
            if N is not None or input_dim is not None or kwargs:
                raise ValueError("Cannot specify both config and individual parameters")
            self.config = config
        else:
            if N is None or input_dim is None:
                raise ValueError("Must provide config or (N and input_dim)")
        ...
        # weights
        self.Wr, self.Wx, self.b = (
            self._create_reservoir_weights(),
            self._create_input_weights(),
            self._create_bias(),
        )
    ...
    def _sparse_spectral_radius(self, J) -> float:
        try:
            ...
            try:
                ...
                vals, _ = sparse_eigs(
                    J, k=1, which="LM", tol=1e-4, maxiter=J.shape[0] * 20, v0=v0
                )
                return float(np.max(np.abs(vals)))
    ...
    def _create_small_world_weights(self) -> NDArray | csr_matrix:
        ...
        J = csr_matrix(
            (data, (row_indices, col_indices)), shape=(cfg.N, cfg.N), dtype=cfg.dtype
        )
    ...
    def _create_scale_free_weights(self) -> NDArray | csr_matrix:
        ...
        for new_node in range(m + 1, cfg.N):
            ...
            targets = self.rng.choice(
                existing_nodes, size=min(m, new_node), replace=False, p=probs
            )
    ...
    def _create_reservoir_weights(self) -> NDArray | csr_matrix:
        ...
        if cfg.weights_generation_strategy == "Gaussian":
            if self._use_sparse:
                J = sparse_random(
                    cfg.N,
                    cfg.N,
                    density=1 - cfg.sparsity,
                    data_rvs=lambda s: self.rng.normal(
                        0, cfg.spectral_radius / np.sqrt(cfg.N), s
                    ),
                    format="csr",
            ...
            if self._use_sparse:
                J = sparse_random(
                    ...
                    cfg.N,
                    density=1 - cfg.sparsity,
                    data_rvs=lambda s: self.rng.uniform(-1, 1, s),
                    format="csr",
                    random_state=self.rng,
                ).astype(cfg.dtype)
                if not cfg.self_connections:
            ...
            if self._use_sparse:
                J = sparse_random(
                    ...
                    cfg.N,
                    density=1 - cfg.sparsity,
                    data_rvs=lambda s: self.rng.choice([-1, 1], size=s, p=[0.5, 0.5]),
                    format="csr",
                    random_state=self.rng,
                ).astype(cfg.dtype)
                if not cfg.self_connections:
                ...
                J = (
                    sparse_random(
                        ...
                        cfg.N,
                        density=1 - cfg.sparsity,
                        data_rvs=lambda s: self.rng.choice(
                            [-1, 1], size=s, p=[0.5, 0.5]
                        ),
                        random_state=self.rng,
                    )
                    .toarray()
            ...
            J = cfg.weights_generation_strategy(
                self.rng,
                cfg.N,
                cfg.N,
                cfg.spectral_radius,
                cfg.sparsity,
                cfg.self_connections,
            )
            if not cfg.self_connections:
                J.setdiag(0)
            J.eliminate_zeros()
        ...
        elif cfg.weights_generation_strategy == "Small-World":
            return self._create_small_world_weights()
        elif cfg.weights_generation_strategy == "Scale-Free":
            return self._create_scale_free_weights()
        else:
            raise ValueError(
                f"unknown weights generation strategy: {cfg.weights_generation_strategy}"
            )

    def _create_input_weights(self) -> NDArray:
        """create input weight matrix"""
        cfg = self.config
        if cfg.input_generation_strategy == "Gaussian":
            return self.rng.normal(0, cfg.input_scaling, (cfg.N, cfg.input_dim)).astype(
                cfg.dtype
            )
        elif cfg.input_generation_strategy == "Uniform":
            return self.rng.uniform(
                -cfg.input_scaling, cfg.input_scaling, (cfg.N, cfg.input_dim)
            ).astype(cfg.dtype)
        elif cfg.input_generation_strategy == "Bernoulli":
            return (
                self.rng.choice(
                    [-1, 1], size=(cfg.N, cfg.input_dim), p=[0.5, 0.5]
                ).astype(cfg.dtype)
                * cfg.input_scaling
            )
        elif callable(cfg.input_generation_strategy):
            return cfg.input_generation_strategy(
                self.rng, cfg.N, cfg.input_dim, cfg.input_scaling
            )
        else:
            raise ValueError(
                f"unknown input generation strategy: {cfg.input_generation_strategy}"
            )

    def _create_bias(self) -> NDArray:
        """create bias vector"""
        cfg = self.config
        if cfg.bias_generation_strategy == "Gaussian":
            return self.rng.normal(0, cfg.bias_scaling, cfg.N).astype(cfg.dtype)
        elif cfg.bias_generation_strategy == "Uniform":
            return self.rng.uniform(-cfg.bias_scaling, cfg.bias_scaling, cfg.N).astype(
                cfg.dtype
            )
        elif cfg.bias_generation_strategy == "Bernoulli":
            return (
                self.rng.choice([-1, 1], size=cfg.N, p=[0.5, 0.5]).astype(cfg.dtype)
                * cfg.bias_scaling
            )
        elif callable(cfg.bias_generation_strategy):
            return cfg.bias_generation_strategy(self.rng, cfg.N, cfg.bias_scaling)
        else:
            raise ValueError(
                f"unknown bias generation strategy: {cfg.bias_generation_strategy}"
            )

    def _compute_preactivation(self, x: NDArray) -> NDArray:
        """compute preactivation (same for all networks)"""
        if self._use_sparse:
            return self.Wr.dot(self.r) + self.Wx @ x + self.b
    ...
    def step(self, x: NDArray) -> NDArray:
        ...
        if x.shape != (self.input_dim,):
            raise ValueError(
                f"input x must have shape ({self.input_dim},), got {x.shape}"
            )
    ...
    def reset_state(self, state: NDArray | None = None):
        ...
        if state is None:
            ...
            if state.shape != (self.N,):
                raise ValueError(
                    f"state must have shape ({self.N},), got {state.shape}"
                )
    ...
    def train(
        ...
        x_train: NDArray,
        washout: int = 100,
        skip_indices: NDArray | None = None,
        skip_window: int = 20,
        ...
        if x_train.ndim != 2:
            raise ValueError(
                f"x_train must be 2D array of shape (input_dim, T), got shape {x_train.shape}"
            )
        if x_train.shape[0] != self.input_dim:
            raise ValueError(
                f"x_train first dimension must match input_dim ({self.input_dim}), got {x_train.shape[0]}"
            )
        ...
        if washout >= T - 1:
            raise ValueError(
                f"washout ({washout}) must be less than T-1 ({T - 1}) to have training samples"
            )

        # validate skip_window
        if not isinstance(skip_window, (int, np.integer)) or skip_window < 0:
            raise ValueError(
                f"skip_window must be a non-negative integer, got {skip_window}"
            )
        ...
        states_with_bias = np.vstack(
            [states, np.ones((1, effective_T), dtype=self.config.dtype)]
        )
        targets = x_train[:, washout + 1 : washout + 1 + effective_T]

        # handle skip indices
        if skip_indices is not None:
            mask = self._compute_skip_mask(
                skip_indices, washout, effective_T, skip_window
            )
    ...
    def _compute_skip_mask(
        self, skip_indices: NDArray, washout: int, effective_T: int, skip_window: int
    ) -> NDArray:
        ...
        for center in skip_centers:
            for offset in range(-skip_window, skip_window + 1):
                idx = center + offset
                if 0 <= idx < effective_T:
                    skip_set.add(idx)

        mask = np.ones(effective_T, dtype=bool)
        if skip_set:
            mask[list(skip_set)] = False
        return mask
    ...
    def _solve_ridge(self, states_with_bias: NDArray, targets: NDArray) -> None:
        ...
        try:
            ...
            ridge = Ridge(
                alpha=self.config.alpha, fit_intercept=False, solver="sparse_cg"
            )
            ridge.fit(states_with_bias.T, targets.T)
            Wout_full = ridge.coef_

        if Wout_full.ndim == 1:
            Wout_full = Wout_full[np.newaxis, :]

        self.Wout = Wout_full[:, :-1]
        self.Wout_bias = Wout_full[:, -1]

    def predict(
        self, warmup: NDArray, steps: int, return_states: bool = True
        ...
        if not self.is_trained:
            raise RuntimeError(
                "ESN must be trained before prediction. Call train() first."
            )
        ...
        if warmup.ndim != 2:
            raise ValueError(
                f"warmup must be 2D array of shape (input_dim, warmup_length), got shape {warmup.shape}"
            )
        if warmup.shape[0] != self.input_dim:
            raise ValueError(
                f"warmup first dimension must match input_dim ({self.input_dim}), got {warmup.shape[0]}"
            )
        if warmup.shape[1] == 0:
        ...
        states = (
            np.zeros((self.N, steps), dtype=self.config.dtype)
            if return_states
            else None
        )
    ...
    def predict_driven(self, data: NDArray) -> tuple[NDArray, NDArray]:
        ...
        if data.ndim != 2:
            raise ValueError(
                f"data must be 2D array of shape (input_dim, steps), got shape {data.shape}"
            )
        if data.shape[0] != self.input_dim:
            raise ValueError(
                f"data first dimension must match input_dim ({self.input_dim}), got {data.shape[0]}"
            )
        if data.shape[1] == 0:
    ...
    def _lyapunov_sample(
        ...
        for t in range(steps):
            ...
            if (t + 1) % norm_time == 0:
                ...
                R_ii_sum += np.log(
                    np.maximum(np.abs(np.diag(R_qr)), np.finfo(dtype).tiny)
                )
                delta = Q[:, :num_lyaps]
                norm_count += 1
    ...
    def lyapunov_spectrum(
        ...
        if not self.is_trained:
            raise RuntimeError(
                "ESN must be trained before Lyapunov estimation. Call train() first."
            )
        ...
        if initial_data.ndim != 2:
            raise ValueError(
                f"initial_data must be 2D array of shape (input_dim, T), got shape {initial_data.shape}"
            )
        if initial_data.shape[0] != self.input_dim:
            raise ValueError(
                f"initial_data first dimension must match input_dim ({self.input_dim}), got {initial_data.shape[0]}"
            )
        ...
        if not isinstance(transient, (int, np.integer)) or transient < 0:
            raise ValueError(
                f"transient must be a non-negative integer, got {transient}"
            )
        ...
        if initial_data.shape[1] < min_data_length:
            raise ValueError(
                f"initial_data length ({initial_data.shape[1]}) is too short for {num_samples} samples with warmup={warmup}"
            )
        ...
        for sample_idx, (lyap_exps, distance, convergence, norm_count) in enumerate(
            results
        ):
        ...
        return {
            "mean": mean_lyap,
            "std": std_lyap,
            "all_samples": all_lyap_exps_valid,
            "convergence": convergence_history,
            "num_valid_samples": len(all_lyap_exps_valid),
            "max_lyapunov": mean_lyap[0] if len(mean_lyap) > 0 else np.nan,
            "distances": distances,
        }

    def conditional_lyapunov_spectrum(
        self,
        data: NDArray,
        num_lyaps: int | None = None,
        norm_time: int = 10,
        dt: float = 0.01,
        warmup: int = 1000,
        transient: int = 1500,
        ...
        if num_lyaps is None:
            ...
            raise ValueError(
                f"num_lyaps must be a positive integer or None, got {num_lyaps}"
            )
        if num_lyaps > self.N:
            raise ValueError(f"num_lyaps ({num_lyaps}) cannot exceed N ({self.N})")
        ...
        if T < total_needed:
            raise ValueError(
                f"data length ({T}) too short. Need at least {total_needed} timesteps (warmup={warmup} + transient={transient} + {norm_time * 10} measurement steps)"
            )
        ...
        if num_lyaps == 1:
            return self._conditional_lyapunov_fast(
                data, Wr, warmup, transient, norm_time, dt, calculate_convergence
            )

        # Standard path for multiple exponents
        return self._conditional_lyapunov_full(
            data, Wr, num_lyaps, warmup, transient, norm_time, dt, calculate_convergence
        )

    def _conditional_lyapunov_fast(
        self,
        data: NDArray,
        Wr,
        warmup: int,
        ...
        return {
            ...
            "convergence": np.array(convergence)[:, np.newaxis]
            if calculate_convergence
            else None,
            "max_cle": max_cle,
            "sum_cle": max_cle,
    ...
    def _conditional_lyapunov_full(
        self,
        data: NDArray,
        Wr,
        num_lyaps: int,
        ...
        convergence = (
            np.zeros((num_renorms, num_lyaps), dtype=dtype)
            if calculate_convergence
            else None
        )
    ...
    def save(self, path: str):
        ...
        # weights
        if hasattr(self.Wr, "toarray"):
            weights_data = {
                "Wr_sparse": True,
                "Wr_data": self.Wr.data,
                "Wr_indices": self.Wr.indices,
                "Wr_indptr": self.Wr.indptr,
                "Wr_shape": np.array(self.Wr.shape),
            }
        else:
            weights_data = {"Wr_sparse": False, "Wr": self.Wr}
    ...
    def load(cls, path: str) -> "ESN":
        ...
        if dynamics_mode == "standard":
            dynamics = StandardDynamics()
        elif dynamics_mode == "leaky":
            dynamics = LeakyDynamics(leaky_rate=data["dynamics_leaky_rate"])
        elif dynamics_mode == "es2n":
            dynamics = ES2NDynamics(beta=data["dynamics_beta"], O=data["dynamics_O"])
        else:
            raise ValueError(f"unknown mode: {dynamics_mode}")
        ...
        if "rng_state" in data:
            try:
                esn.rng.bit_generator.state = json.loads(data["rng_state"].item())
        ...
        if data["Wr_sparse"]:
            esn.Wr = csr_matrix(
                (data["Wr_data"], data["Wr_indices"], data["Wr_indptr"]),
                shape=tuple(data["Wr_shape"]),
            )
        else:
            esn.Wr = data["Wr"]
        ...
        if "Wout" in data:
            ...
            esn.Wout, esn.Wout_bias = None, None
