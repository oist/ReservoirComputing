...
class SearchParam:
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.bounds, tuple):
            raise ValueError(
                f"bounds must be a tuple, got {type(self.bounds).__name__}"
            )

        if self.param_type == "categorical":
            if len(self.bounds) < 2:
                raise ValueError(
                    f"categorical bounds must have at least 2 choices, got {len(self.bounds)}"
                )
        else:
            if len(self.bounds) != 2:
                raise ValueError(
                    f"bounds must be a tuple of 2 values (low, high), got {len(self.bounds)} values"
                )
        ...
        if self.param_type not in valid_types:
            raise ValueError(
                f"param_type must be one of {valid_types}, got '{self.param_type}'"
            )
...
class ESNSearchSpace:
    ...
    def __post_init__(self):
        ...
        if unknown_opt:
            raise ValueError(f"unknown optimize: {unknown_opt}")
        unknown_fix = set(self.fixed) - self._valid_fixed
        if unknown_fix:
            raise ValueError(f"unknown fixed: {unknown_fix}")
    ...
    def to_ax_params(self) -> list[dict]:
        ...
        for k, v in self.optimize.items():
            if v.param_type == "categorical":
                params.append(
                    {
                        "name": k,
                        "type": "choice",
                        "values": list(v.bounds),
                        "is_ordered": False,
                        "sort_values": False,
                    }
                )
                ...
                params.append(
                    {
                        "name": k,
                        "type": "range",
                        "value_type": v.param_type,
                        "bounds": list(v.bounds),
                        "log_scale": v.log_scale,
                    }
                )
        return params
    ...
    def create_ax_client(
        ...
        ax.create_experiment(
            ...
            choose_generation_strategy_kwargs={
                ...
                "min_sobol_trials_observed": int(num_initialization_trials / 2)
                if num_initialization_trials
                else None,
            },
        )
...
class ESNSearchSpaceBuilder:
    ...
    DEFAULTS: ClassVar[dict[str, SearchParam]] = {
        ...
        "weights_generation_strategy": SearchParam(
            ("Gaussian", "Uniform", "Bernoulli", "Small-World", "Scale-Free"),
            param_type="categorical",
        ),
        "bias_generation_strategy": SearchParam(
            ("Gaussian", "Uniform", "Bernoulli"), param_type="categorical"
        ),
        "input_generation_strategy": SearchParam(
            ("Gaussian", "Uniform", "Bernoulli"), param_type="categorical"
        ),
    ...
    def optimize(
        self, name: str, bounds: tuple | None = None, log_scale: bool | None = None
        ...
        ESNSearchSpaceBuilder
            Builder instance.
        """
        if name not in self.DEFAULTS and bounds is None:
    ...
    def build(self) -> ESNSearchSpace:
        ...
        logger.debug(
            f"Built search space with {len(self._optimize)} optimizable parameters and {len(self._fixed)} fixed parameters"
        )
...
class EvaluationConfig:
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.washout, int) or self.washout < 0:
            raise ValueError(
                f"washout must be a non-negative integer, got {self.washout}"
            )
        if not isinstance(self.warmup_steps, int) or self.warmup_steps <= 0:
            raise ValueError(
                f"warmup_steps must be a positive integer, got {self.warmup_steps}"
            )
        if not isinstance(self.predict_steps, int) or self.predict_steps <= 0:
            raise ValueError(
                f"predict_steps must be a positive integer, got {self.predict_steps}"
            )
        if not isinstance(self.n_predictions, int) or self.n_predictions <= 0:
            raise ValueError(
                f"n_predictions must be a positive integer, got {self.n_predictions}"
            )
        if not isinstance(self.n_instances, int) or self.n_instances <= 0:
            raise ValueError(
                f"n_instances must be a positive integer, got {self.n_instances}"
            )
        if not isinstance(self.n_jobs, int):
            raise ValueError(
                f"n_jobs must be an integer, got {type(self.n_jobs).__name__}"
            )
        if (
            not isinstance(self.wasserstein_projections, int)
            or self.wasserstein_projections <= 0
        ):
            raise ValueError(
                f"wasserstein_projections must be a positive integer, got {self.wasserstein_projections}"
            )
        ...
        if not isinstance(self.vpt_threshold, (int, float)) or self.vpt_threshold <= 0:
            raise ValueError(
                f"vpt_threshold must be positive, got {self.vpt_threshold}"
            )
        if (
            not isinstance(self.clip_wasserstein, (int, float))
            or self.clip_wasserstein <= 0
        ):
            raise ValueError(
                f"clip_wasserstein must be positive, got {self.clip_wasserstein}"
            )
        ...
        if invalid:
            raise ValueError(
                f"invalid metrics: {invalid}. Valid metrics are: {valid_metrics}"
            )
...
class ESNObjective:
    ...
    def __init__(
        ...
        if not isinstance(space, ESNSearchSpace):
            raise TypeError(
                f"space must be an ESNSearchSpace instance, got {type(space).__name__}"
            )
        if not isinstance(config, EvaluationConfig):
            raise TypeError(
                f"config must be an EvaluationConfig instance, got {type(config).__name__}"
            )
        ...
        self.starts = self._get_stratified_starts(
            n_total=config.n_predictions, low=config.washout, high=high, seed=seed
        )
        time_end = time.time()
        logger.debug(
            f"Time taken to get stratified starts: {time_end - time_start:.2f} seconds"
        )

        time_start = time.time()
        if self._needs_wasserstein:
            self.projections = self._get_projections(
                self.D, config.wasserstein_projections, seed=seed, method="orthogonal"
            )
        else:
            self.projections = None
        time_end = time.time()
        logger.debug(
            f"Time taken to get projections: {time_end - time_start:.2f} seconds"
        )
        ...
        time_end = time.time()
        logger.debug(
            f"Time taken to precompute windows: {time_end - time_start:.2f} seconds"
        )

    def _get_stratified_starts(
        self, n_total: int, low: int, high: int, seed: int
    ) -> NDArray:
        rng = np.random.default_rng(seed)
        edges = np.linspace(low, high, n_total + 1)
        starts = np.array(
            [rng.integers(int(edges[i]), int(edges[i + 1])) for i in range(n_total)]
        )
        rng.shuffle(starts)
        return starts

    def _get_projections(
        self, d: int, n: int, seed: int, method: str = "orthogonal"
    ) -> NDArray:
        rng = np.random.default_rng(seed)
    ...
    def _precompute_windows(self):
        ...
        self.warmup_windows = np.empty(
            (n_windows, self.D, cfg.warmup_steps), dtype=self.data.dtype
        )
        self.gt_windows = np.empty(
            (n_windows, self.D, cfg.predict_steps), dtype=self.data.dtype
        )

        for i, start in enumerate(self.starts):
            self.warmup_windows[i] = self.data[:, start : start + cfg.warmup_steps]
            self.gt_windows[i] = self.data[
                :,
                start + cfg.warmup_steps : start + cfg.warmup_steps + cfg.predict_steps,
            ]
    ...
    def _compute_wasserstein(
        self, pred: NDArray, window_idx: int, p: int = 2
    ) -> tuple[float, float]:
        time_start = time.time()
        ...
        time_end = time.time()
        logger.debug(
            f"Time taken to project and sort predictions: {time_end - time_start:.2f} seconds"
        )
        ...
        time_end = time.time()
        logger.debug(
            f"Time taken to compute Wasserstein distance: {time_end - time_start:.2f} seconds"
        )
        ...
        time_end = time.time()
        logger.debug(
            f"Time taken to compute max and mean Wasserstein distance: {time_end - time_start:.2f} seconds"
        )
    ...
    def _eval_instance(self, params: dict, idx: int) -> dict:
        ...
        try:
            ...
            time_end = time.time()
            logger.debug(
                f"Time taken to train ESN: {time_end - time_start:.2f} seconds"
            )
        ...
        if "max_cle" in self.config.metrics or self.config.constrain_cle:
            try:
                max_cle = calculate_max_conditional_lyapunov_exponent(
                    esn, self.data, self.config.dt, self.config.length
                )
        ...
        time_end = time.time()
        logger.debug(
            f"Time taken to compute max CLE: {time_end - time_start:.2f} seconds"
        )
            ...
            try:
                ...
                if self._needs_wasserstein:
                    ...
                    if "max_wasserstein" in results:
                        results["max_wasserstein"].append(
                            np.clip(sw_max, 0, self.config.clip_wasserstein)
                        )
                    if "wasserstein" in results:
                        results["wasserstein"].append(
                            np.clip(sw_mean, 0, self.config.clip_wasserstein)
                        )
                    if "max_wasserstein" in compare_results:
                        compare_results["max_wasserstein"].append(
                            np.clip(sw_max, 0, self.config.clip_wasserstein)
                        )
                    if "wasserstein" in compare_results:
                        compare_results["wasserstein"].append(
                            np.clip(sw_mean, 0, self.config.clip_wasserstein)
                        )
                if "vpt" in results:
                    ...
                    results["vpt"].append(
                        valid_prediction_time(
                            gt, pred, self.config.vpt_threshold, self.config.dt
                        )
                    )
            except Exception as e:
        ...
        time_end = time.time()
        logger.debug(
            f"Time taken to evaluate instance {idx}: {time_end - time_start:.2f} seconds"
        )
        return {k: np.median(v) for k, v in results.items() if v}, {
            k: np.median(v) for k, v in compare_results.items() if v
        }
...
def _generate_grid(
    space: ESNSearchSpace, points: dict[str, int | list[float]] | int
) -> list[dict]:
    ...
    if isinstance(points, int):
        ...
        for k, v in points.items():
            if k not in space.optimize:
                raise ValueError(
                    f"unknown parameter in points: '{k}'. Valid parameters: {list(space.optimize.keys())}"
                )
    ...
    for name, search_param in space.optimize.items():
        ...
        elif search_param.param_type == "categorical":
            param_values[name] = list(search_param.bounds)
        else:
            low, high = search_param.bounds
            if search_param.log_scale:
                param_values[name] = np.geomspace(low, high, spec).tolist()
            else:
                param_values[name] = np.linspace(low, high, spec).tolist()
...
def optimize_esn(
    ...
    if method == "grid":
        ...
        for i, params in enumerate(grid):
            ...
            comparison[:, i] = [compare_result[m][0] for m in compare_metrics] + [
                result[m][0] for m in config.metrics
            ]
