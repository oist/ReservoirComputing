...
class LeakyDynamics:
    ...
    >>> dynamics = create_dynamics("leaky", N=100, leaky_rate=0.2)
    """
...
class ES2NDynamics:
    ...
    >>> dynamics = create_dynamics("es2n", N=100, beta=0.5)
    """
...
def create_dynamics(mode: Literal["standard", "leaky", "leakyrand", "es2n", "es2nrand"] | str, N: int, dtype: np.dtype = np.float64, 
    ...
    """
    if mode == "standard": return StandardDynamics()
    elif mode == "leaky":
        lr = np.full(N, leaky_rate, dtype=dtype) if np.isscalar(leaky_rate) else np.asarray(leaky_rate, dtype=dtype)
        return LeakyDynamics(leaky_rate=np.clip(lr, 0, 1))
    elif mode == "es2n":
        b = np.full(N, beta, dtype=dtype) if np.isscalar(beta) else np.asarray(beta, dtype=dtype)
...
class ESN:
    ...
    def _create_small_world_weights(self) -> NDArray | csr_matrix:
        ...
        Uses sparsity to determine k (neighbors), and a fixed rewiring probability.
        """
        cfg = self.config
    ...
    def _create_scale_free_weights(self) -> NDArray | csr_matrix:
        """Barabási-Albert scale-free reservoir.
        
        Uses sparsity to determine m (edges per new node).
        """
        cfg = self.config
    ...
    def reset_state(self, state: NDArray | None = None):
        ...
        """
        if state is None: self.r = self.rng.uniform(-1, 1, self.N).astype(self.config.dtype)
        else: self.r = state.astype(self.config.dtype)
    ...
    def predict(self, warmup: NDArray, steps: int) -> tuple[NDArray, NDArray]:
        ...
        """
        if not self.is_trained:raise RuntimeError("ESN must be trained before prediction")
    ...
    def lyapunov_spectrum(self, initial_data: NDArray, num_lyaps: int = 40, steps: int = 10000, norm_time: int = 10, dt: float = 0.25, num_samples: int = 5, warmup: int = 100, transient: int = 100, calculate_convergence: bool = False) -> dict:
        ...
        """
        if not self.is_trained: raise RuntimeError("ESN must be trained before Lyapunov estimation")
        if num_lyaps > self.N: raise ValueError(f"num_lyaps ({num_lyaps}) cannot exceed N ({self.N})")
    ...
    def conditional_lyapunov_spectrum(self, data: NDArray, num_lyaps: int | None = None, norm_time: int = 10, dt: float = 0.01, warmup: int = 1000, transient: int = 1500, calculate_convergence: bool = False) -> dict:
        ...
        """
        T = data.shape[1]
        if num_lyaps is None: num_lyaps = self.N
        if num_lyaps > self.N: raise ValueError(f"num_lyaps ({num_lyaps}) cannot exceed N ({self.N})")
        
        total_needed = warmup + transient + norm_time * 10  # at least 10 renormalizations
        if T < total_needed: raise ValueError(f"data length {T} too short. need at least {total_needed}")
        
        dtype = self.config.dtype
        Wr = self.Wr.toarray() if hasattr(self.Wr, 'toarray') else self.Wr
        
        # warmup: drive reservoir to get on attractor
        self.reset_state()
        for i in range(warmup):
            self.step(data[:, i])
        
        # store reservoir states for lyapunov computation
        ...
        for i in range(measure_steps):
            z = self._compute_preactivation(data[:, warmup + i])
            self.r = self.dynamics.update(self.r, z, self._activation)
            states[:, i] = self.r
            preactivations[:, i] = z
        ...
        # transient: let tangent vectors align with Lyapunov directions
        for i in range(transient // norm_time):
            for j in range(norm_time):
                idx = i * norm_time + j
                G, _ = self.dynamics.conditional_jacobian_update(G, states[:, idx], preactivations[:, idx], Wr)
            G, _ = np.linalg.qr(G, mode='reduced')
        ...
        if calculate_convergence: convergence = np.zeros((num_renorms, num_lyaps), dtype=dtype)
        ...
        for i in range(num_renorms):
            ...
            # running estimate
            if calculate_convergence: convergence[i] = R_log_sum / ((i + 1) * norm_time * dt)
        ...
        return {'exponents': exponents, 'convergence': convergence[:, sort_idx] if calculate_convergence else None, 'max_cle': exponents[0], 'sum_cle': np.sum(exponents), 'num_renorms': num_renorms}
